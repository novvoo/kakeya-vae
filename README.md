...
将这一“方向覆盖 + 紧凑支撑”的几何直觉迁移到图像压缩，得到当前核心约束：
用固定的多方向、多尺度细管核比较目标与重建的线段响应，保留文字、枝条和轮廓
的方向与连续性，同时惩罚目标不支持的伪线和光晕。该约束只作用于 256 及以下；
384 及以上高清路径保持原有训练目标。当前工程只维护图像压缩模型，不再提供
通用 VAE 变体的训练入口。
...
| 参数 | 使用方法 | 作用 |
|---|---|---|
| 细管方向数 | 图文模型、超先验 Kakeya | 固定抗锯齿线核的方向数量，默认 12 |
| 细管尺度数 | 图文模型 | 启用长度 5/9/17 中的前几个尺度，默认 3 |
| 率失真 λ | 超先验 Kakeya VAE | 控制码率与失真的权衡 |
| 挂谷 λₖ | 超先验 Kakeya VAE | 控制挂谷覆盖正则强度 |

训练会按输入尺寸调整细节目标：256×256 及以下降低率惩罚、适度加强边缘与结构损失，
并以 Laplacian 高频监督配合平滑区域 anti-ringing 约束，避免色块伪纹理和过锐光晕；
384×384 及以上保持原有率失真权重。最佳 checkpoint
...
`[-3, 3]`。Kakeya Tube Loss 在重建图上计算并参与反向传播；box-counting
维数代理包含硬占据计数，只用于监控，不进入总损失。训练结束后不会盲目使用
最后一轮，而是在预算内选择校准 PSNR 最高的 checkpoint，再熵编码、写入 `.kky`、
解码同一份负载生成报告图。
