...
最近一次 256×256 图文模型训练（运行 ID `20260726T062428Z`，Apple MPS，
50 轮）。容量阶段第 1–39 轮，第 39 轮通过闸门（PSNR 30.17 dB、SSIM 0.9838）；
压缩微调第 40–50 轮，最终选用第 50 轮 checkpoint 生成 `.kky` 码流。

### 1. 损失收敛曲线

![损失收敛曲线](assets/loss_curve_20260726T062428Z.png)

| 阶段 | 轮次 | 训练损失 | 验证损失 | 独立图文 | 校准 PSNR | 校准 SSIM |
|---|---|---:|---:|---:|---:|---:|
| 容量阶段起点 | 1 | 1.6532 | 1.2629 | 1.2782 | 12.58 | 0.4868 |
| 容量阶段末 | 39 | 0.1076 | 0.1100 | — | 30.17 | 0.9838 |
| 压缩微调起点 | 40 | 0.7420 | — | — | — | — |
| 末轮 | 50 | 0.2179 | 0.2028 | 0.2028 | 31.75 | 0.9851 |

容量阶段训练/验证损失从 ~1.65 平滑下降到 ~0.11，PSNR 从 12.58 dB 升至
30.17 dB，SSIM 从 0.49 升至 0.98，收敛稳定。第 40 轮切到压缩微调后因引入
程序化图文与 rate loss，训练损失回升到 0.74，随后 11 轮内回落到 0.22，
PSNR 继续提升到 31.75 dB，未出现画质坍塌。
...
<table>
  <tr>
    <td align="center"><img src="assets/reconstruction_original_20260726T062428Z.png" alt="原图" width="256"></td>
    <td align="center"><img src="assets/reconstruction_result_20260726T062428Z.png" alt="模型还原图" width="256"></td>
    <td align="center"><img src="assets/reconstruction_error_20260726T062428Z.png" alt="误差热图" width="256"></td>
  </tr>
  <tr>
...
还原 PSNR 31.75 dB、SSIM 0.9851，文字与细线区域保真度良好。

### 3. 同一图文测试卡编码对比

![编码对比](assets/codec_comparison_20260726T062428Z.png)
...
| **Kakeya VAE** | `.kky` 码流 | **5 605** | **0.000668** | **31.75** | **0.9851** |
...
| **Kakeya VAE** | 128 张程序化图文卡 | 50 轮（容量阶段 39 轮 + 压缩微调 11 轮） |

Kakeya 以极小的训练规模（128 样本 × 50 轮）在图文领域达到了与 mbt2018 q6
（大规模自然图像训练）相当的画质、且体积仅为其三分之一，说明图文专项训练
在细线 / 文字结构上的数据效率很高。

### 4. 总结

`.kky` 码流 5 605 B（0.6842 bpp），相比原始 PNG 节省 95.1% 字节，PSNR
31.75 dB、SSIM 0.9851 满足容量闸门（PSNR ≥ 30、SSIM ≥ 0.97）。

**与传统编码对比（同图实测）：** 与 WebP q90（23 782 B、35.57 dB、0.9914）
相比，Kakeya 码流字节仅为 WebP 的 23.6%，但 PSNR 低 3.82 dB、SSIM 低
0.0063——即以不到 WebP 四分之一的体积逼近其画质，质量体积权衡显著占优，
但绝对画质仍有差距。

**与学习式编码基线对比（同图实测）：** 与 CompressAI 官方预训练 mbt2018
相比：
- mbt2018 q2（5 236 B、21.61 dB）：与 Kakeya 码流大小接近，但 Kakeya PSNR
  高出 10.14 dB、SSIM 高出 0.084——因为 mbt2018 在自然图像上预训练，
  对图文这种高频细线结构不擅长，而 Kakeya 专门在图文卡上训练。
- mbt2018 q6（16 676 B、31.46 dB）：PSNR 与 Kakeya 相当（31.46 vs 31.75 dB），
  但字节是 Kakeya 的 3.0 倍。Kakeya 用更小的体积在图文领域达到了
  超先验模型中高码率点的画质。
...
当前差距主要来自训练规模：本模型仅在 128 张程序化图文卡上训练 50 轮，
...
输入需为 256×256 RGB 图片，非该尺寸会自动缩放。输出 `.kky` 码流的实际
字节数和 PSNR 会在终端打印。
...
| `reports/reconstruction.kky` | 单张测试卡的压缩码流 | 约 5.5 KB |
| `reports/reconstruction.png` | 从 `.kky` 解码还原的图片 | 约 130 KB |

## 256×256 图文测试

网页默认选择“256 图文 Kakeya VAE”，并使用适合 Mac MPS 的起始参数
（最大 50 轮、128 张训练卡、batch size 4、空间潜在通道 8），首页直接显示
内置测试图。验证仅在首轮、偶数轮和末轮执行，验证集最多 64 张；容量阶段固定
`batch size = 1`、每轮 32 个优化步骤，避免把同一张卡复制成大 batch 消耗
内存却没有增加更新次数。默认最大轮次提高到 50 后训练时间会相应增加；模型一旦通过
容量闸门就会自动切换到完整图文数据和压缩微调，不需要手工重新启动。
