...
31.75 dB、SSIM 0.9851 满足容量闸门（PSNR ≥ 30、SSIM ≥ 0.97）。与 WebP q90
（23 782 B、35.57 dB、0.9914）相比，Kakeya 码流字节仅为 WebP 的 23.6%，
但 PSNR 低 3.82 dB、SSIM 低 0.0063——即以不到 WebP 四分之一的体积逼近其
画质，质量体积权衡显著占优，但绝对画质仍有差距。

理论上 Kakeya VAE 相比 WebP 具备结构性优势：

- **内容自适应表征**：WebP 使用固定的 4×4 DCT 块变换与预设量化表，对文字
  边缘和细线区域易产生振铃与块效应；Kakeya VAE 的 32×32 空间潜变量与
  PixelUnshuffle 降采样保留了像素级细节，且熵瓶颈层对潜变量做内容自适应
  量化，理论上能更紧凑地编码高频图文结构。
- **端到端率失真优化**：WebP 的码率控制与量化是分阶段启发式策略，而
  Kakeya VAE 的 rate loss 直接对熵模型估计的 bpp 做梯度回传，可在给定
  预算下逼近全局率失真最优。

当前差距主要来自训练规模：本模型仅在 128 张程序化图文卡上训练 50 轮，
WebP/JPEG 则基于大规模自然图像库长期调优。未来可通过以下方向超越 WebP：

1. **扩大训练集**：引入真实图文、文档、UI 截图等高频细线样本，覆盖 WebP
   训练分布之外的图像类型，提升泛化画质；
2. **感知损失升级**：用 LPIPS / DISTS 等感知指标替换或加权 MSE，让模型在
   相同 bpp 下更贴近人眼画质判断；
3. **熵模型增强**：从 EntropyBottleneck 升级到超先验或上下文模型
  （如 CompressAI 的 MeanScaleHyperprior），更精确地捕捉潜变量空间相关性，
   进一步压低 bpp；
